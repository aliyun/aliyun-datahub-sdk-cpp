...
class PutRecordGTest: public testing::Test
{
protected:
    virtual void SetUp()
    {
        ...
        Configuration conf(account, conf_endpoint, false);
    ...
    static RecordEntry GenRecordEntryWithNull(const RecordSchema& schema)
    {
        RecordEntry entry(schema.GetFieldCount());
        entry.SetString(schema.GetFieldIndex("a"), "");
        entry.SetBigint(schema.GetFieldIndex("b"), 5L);
        entry.SetDouble(schema.GetFieldIndex("c"), 0.0);
        entry.SetTimestamp(schema.GetFieldIndex("d"), 123456789000000L);
        entry.SetBoolean(schema.GetFieldIndex("e"), true);
...
DatahubClient* PutRecordGTest::m_client;
std::string PutRecordGTest::m_project;
std::string PutRecordGTest::m_topic;
std::string PutRecordGTest::m_blob_topic;
RecordSchema PutRecordGTest::m_schema;

TEST_F(PutRecordGTest, PutRecordTest)
{
...
TEST_F(PutRecordGTest, PutRecordWithNullTest)
{
    GetTopicResult getTopicResult = m_client->GetTopic(m_project, m_topic);
    RecordSchema schema = getTopicResult.GetRecordSchema();
    sleep(5);
    ...
    for (int i = 0; i < recordNumber; ++i)
    {
        RecordEntry entry = GenRecordEntryWithNull(schema);
        std::stringstream ss;
        ss << i % SHARDCOUNT;
        const std::string& shardId = ss.str();
        entry.SetShardId(shardId);
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        ASSERT_NE(recordResult.GetSystemTime(), -1);
        ASSERT_NE(recordResult.GetSequence(), -1);
        ASSERT_EQ(recordResult.GetString(0), "");
        ASSERT_EQ(recordResult.GetBigint(1), 5L);
        ASSERT_EQ(recordResult.GetDouble(2), 0.0);
        ASSERT_EQ(recordResult.GetTimestamp(3), 123456789000000L);
        ASSERT_EQ(recordResult.GetBoolean(4), true);
        ASSERT_EQ(recordResult.GetDecimal(5), "123.123");
        ASSERT_EQ(recordResult.GetInteger(6), 123);
        ASSERT_TRUE(recordResult.IsNull(7));
        ASSERT_THROW(recordResult.GetFloat(7), DatahubException);
    }
...
TEST_F(PutRecordGTest, PutRecordFailedTest)
{
    sleep(5);
    GetTopicResult getTopicResult = m_client->GetTopic(m_project, m_topic);
    RecordSchema schema = getTopicResult.GetRecordSchema();
...
TEST_F(PutRecordGTest, PutBlobRecordTest)
{
    sleep(5);
...
TEST_F(PutRecordGTest, PutNullRecordTest)
{
    sleep(5);
    GetTopicResult getTopicResult = m_client->GetTopic(m_project, m_topic);
    RecordSchema schema = getTopicResult.GetRecordSchema();
...
TEST_F(PutRecordGTest, PutRecordLZ4Test)
{
    sleep(5);
...
TEST_F(PutRecordGTest, PutBlobRecordLZ4Test)
{
    sleep(5);
    compressType = compress::LZ4;
    GetTopicResult getTopicResult = m_client->GetTopic(m_project, m_blob_topic);
...
TEST_F(PutRecordGTest, PutRecordZLIBTest)
{
    sleep(5);
...
TEST_F(PutRecordGTest, PutBlobRecordZLIBTest)
{
    sleep(5);
    compressType = compress::ZLIB;
    GetTopicResult getTopicResult = m_client->GetTopic(m_project, m_blob_topic);
...
TEST_F(PutRecordGTest, PutRecordDeflateTest)
{
    sleep(5);
...
TEST_F(PutRecordGTest, PutBlobRecordDeflateTest)
{
    sleep(5);
    compressType = compress::DEFLATE;
    GetTopicResult getTopicResult = m_client->GetTopic(m_project, m_blob_topic);
