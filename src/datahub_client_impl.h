#ifndef DATAHUB_SDK_DATAHUB_CLIENT_IMPL_H
#define DATAHUB_SDK_DATAHUB_CLIENT_IMPL_H
...
#include "datahub/sdk_version.h"

#define VERSION_MAJOR    1    /* for breaking interface changes */
#define VERSION_MINOR    0    /* for new (non-breaking) interface capabilities */
#define VERSION_RELEASE  0    /* for tweaks, bug-fixes, or development */
...
public:
     ...
     */
    virtual ListProjectResult ListProject() = 0;
     ...
     */
    virtual void CreateProject(const std::string& project, const std::string& comment) = 0;
     ...
     *        The name of the project.
     */
    virtual void DeleteProject(const std::string& project) = 0;
     ...
     */
    virtual GetProjectResult GetProject(const std::string& project) = 0;
     ...
     *         The description of the project.
     */
    virtual void UpdateProject(const std::string& project, const std::string& comment) = 0;
     ...
     */
    virtual ListTopicResult ListTopic(const std::string& project) = 0;
     ...
     *        Unit of time represents in day.
     * @param type
     *        The type of the topic, now support BLOB only.
     * @param comment
     *        The desc for the topic.
     */
    virtual void CreateTopic(const std::string& project,
     ...
     *        The name of the topic.
     */
    virtual void DeleteTopic(const std::string& project, const std::string& topic) = 0;
     ...
     */
    virtual GetTopicResult GetTopic(const std::string& project, const std::string& topic) = 0;
    ...
    virtual ListShardResult ListShard(
            const std::string& project, const std::string& topic) = 0;
    ...
    virtual void ActivateShard(const std::string& project,
            const std::string& topic,
            const std::vector<std::string>& shards) = 0;
    ...
    virtual MergeShardResult MergeShard(const std::string& project,
            const std::string& topic,
            const std::string& shardId,
            const std::string& adjacentShardId) = 0;
    ...
    virtual PutRecordResult PutRecord(
            const std::string& project,
            const std::string& topic,
            const std::vector<RecordEntry>& records) = 0;
    ...
    virtual GetConnectorResult GetConnector(
            const std::string& project,
            const std::string& topic,
            const std::string& connectorId) = 0;
    ...
    virtual void UpdateConnectorState(
            ...
            const std::string& topic,
            const std::string& connectorId,
            const sdk::ConnectorState& state) = 0;
    ...
    virtual void UpdateConnectorOffset(
            ...
            const std::string& topic,
            const std::string& connectorId,
            const sdk::ConnectorOffset& connectorOffset) = 0;
    ...
    virtual void UpdateSubscriptionState(
            ...
            const std::string& topic,
            const std::string& subId,
            const SubscriptionState& state) = 0;

    /**
     * Open a subscription session and get subscription offsets.
     *
     * @param project
     *         The name of the project.
     * @param topic
    ...
    virtual void UpdateSubscriptionOffset(
            ...
            const std::string& topic,
            const std::string& subId,
            const std::map<std::string, SubscriptionOffset>& offsets) = 0;
    ...
    virtual HeartbeatResult Heartbeat(
            ...
            const StringVec& readEndShardList) = 0;
    ...
    */
    ...
    virtual ~DatahubClientImpl();

public:
    DatahubMetric GetReadMetric() const;
    DatahubMetric GetWriteMetric() const;

protected:
    static bool IsResponseOK(const http::HttpResponse& response);

    mutable DatahubMetric mReadMetric;
    mutable DatahubMetric mWriteMetric;
};
